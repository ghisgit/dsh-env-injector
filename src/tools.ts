...
export interface ToolEventSource {
  on(
    ...
    listener: (this: unknown, exec: unknown, result: ToolResultLike, next: () => Promise<ToolDecision>) => Promise<ToolDecision>,
  ): unknown
}
 ...
 */
export function asToolEventSource(view: unknown): ToolEventSource | undefined {
  if (view === null || (typeof view !== 'object' && typeof view !== 'function')) return undefined
  const candidate = view as { on?: unknown }
  return typeof candidate.on === 'function' ? view as ToolEventSource : undefined
}
