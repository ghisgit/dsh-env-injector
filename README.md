...
> 本文件是仓库的默认 README；英文版（[README.en.md](README.en.md)）与它同步维护，
> 两者若有不一致，以英文版为准。
...
**Settings → Plugins → Plugin configuration** 里有一张 **Environment injection**
卡片：规则列表，`overrideExisting`、`terminal`、`logMatches` 三个开关，以及每个字段是否
被用户层覆盖。卡片随本包的浏览器端一起发布（`dsh.client` → `lib/client.js`），因为该标签
按 settings namespace 派发 `settings.plugin.item`，只渲染「owner 在该 key 下注册了卡片」
的那些 namespace。

卡片先暂存你的输入，点 **Save** 才写入；**Discard** 丢弃草稿；字段上的 **Reset** 暂存一次
清除，让该字段重新跟随部署的组合 entry。一次保存只写一条原子 mutation，且只覆盖你改动过的
字段，所以并发修改其它字段不会被覆盖。无法编译的规则——非法正则或 flags、空 `command` 或
空 `envVar`——会在对应字段下报错并阻止保存，而不是白跑一次被拒绝的写入。
...
| web 组合里没有 settings 界面 | host 端行为不变；卡片不会挂载，因为浏览器端在等 `slots`、`locale`、`settingsScope` 三个服务。namespace 仍会出现在 `settings.yaml`，校验方式完全相同。 |
...
pnpm build                      # tsc → lib/，再用 esbuild → lib/client.js（两者都已提交）
pnpm typecheck                  # host 配置 + 浏览器端自己的配置
pnpm test                       # 79 个测试：匹配、注入、实时 settings、浏览器卡片、打包
...
| `src/client/index.tsx` | 浏览器端入口：注册卡片词典、绑定 `env-injector` settings scope，并把卡片贡献到 `settings.plugin.item`。 |
| `src/client/card.tsx`、`src/client/controller.ts` | 卡片与它的表单模型：暂存草稿、每字段覆盖状态、只写被改字段的一次原子保存。 |
| `src/client/contracts.ts` | 客户端对 section 的镜像，以及判断草稿能否写入的纯函数（刻意只与卡片共享，不与 host 端编译器共享）。 |
| `lib/index.js` | 构建产物——`dsh` 实际导入的东西（唯一的运行时 import：`@deepseek-ai/schemastery`）。 |
| `lib/client.js` | 构建产物——客户端模块系统提供的浏览器 bundle。文件头带有 `src/client/**` 的摘要，测试会在两者不一致时失败。 |
| `cordis.patch.yml` | bundle 层：一个 `insert` 行，其组合 entry 携带空的 `rules:` 列表（注释里的示例规则是文档化的起点，不是默认值）。 |
| `scripts/build-client.mjs` | 浏览器端构建：esbuild 打包 → 模块系统加载的 lazy-CJS factory；任何不在客户端 baseline 内的模块都会让构建失败。 |
| `scripts/client-baseline.mjs` | 浏览器端允许解析的平台模块表，构建与测试共用。 |
...
| `test/e2e.test.mjs` | 真实 `dsh-subprocess-local` + 真实 `dsh-settings-file`：真实 `bash` 子进程打印自己的环境、真实 `settings.yaml` 编辑实时改写规则、describe 线上形状按卡片渲染所需断言，旧 provider 回退在真实服务上（去掉 `installSection`）跑通。 |
| `test/client.test.mjs` | 针对已发布 bundle 的浏览器端测试：loader 契约、卡片注册、暂存式表单模型，以及卡片在 jsdom + 真实 React 下的 DOM 行为。 |
| `test/packaging.test.mjs` | 安装相关的声明：bundle patch、`dsh.client` 声明、发布路径、profile 形状的解析与 `unwrapExports`。 |
