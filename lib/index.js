...
export function apply(ctx, config) {
    ...
    const toolsServicePresent = () => ctx.get('tools') !== undefined;
    /** Whether a settings service is mounted at all, namespace resolved or not. */
    const settingsServicePresent = () => ctx.get('settings') !== undefined;
    /**
    ...
    let announcedRules;
    const reportRules = () => {
        if (!settingsAttached && settingsServicePresent())
            return;
